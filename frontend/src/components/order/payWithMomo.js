...
export default class App extends Component {
    ...
    componentDidMount() {
        const item = this.props.navigation.getParam('dataPayment');
        this.setState({
            _idBill: item._id,
    ...
    onPress = async () => {
        if (!this.state.processing) {
            ...
            // // iOS App Only , get from Info.plist > key URL types > URL Schemes. Check Readme

            jsonData._idBill = _idBill;
            jsonData.totalBill = totalBill;
    ...
    render() {
        let { textAmount, description, _idBill, totalBill } = this.state;
        return (
            <SafeAreaView style={{ flex: 1, marginTop: 50, backgroundColor: 'transparent' }}>
                <View style={styles.container}>
                    <View style={[{ backgroundColor: 'transparent', alignItems: 'center', justifyContent: 'center', height: 100 }]}>
                        <Image style={{ flex: 1, width: 100, height: 100 }} source={require('../../res/images/logo/logo-momo.png')} />
                    </View>
                    <Text style={[styles.text, { color: 'red', fontSize: 20 }]}>
                        {"MOMO PAYMENT"}</Text>
                    <Text style={[styles.text, { color: 'red', fontSize: 20 }]}>
                        {"THANH TOÁN ONLINE"}</Text>
                    <Text style={[styles.text, { color: '#000', fontSize: 18, marginVertical: 5, textAlign: 'left', margin: 20 }]}>
                        {"Mã đơn hàng : " + _idBill}</Text>
                    {/* <Text style={[styles.text, { color: '#000', fontSize: 14, marginVertical: 5, textAlign: 'left', margin: 20 }]}>
                        {"MerchantName : " + merchantname}</Text>
                    <Text style={[styles.text, { color: '#000', fontSize: 14, marginVertical: 5, textAlign: 'left', margin: 20 }]}>
                        {"Description : " + billdescription}</Text> */}
                    <View style={styles.formInput}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                            <Text style={{ flex: 1, fontSize: 18, paddingHorizontal: 10 }}>{"Số tiền thanh toán"}</Text>
                            {/* <TextInput
                                autoFocus={true}
                                maxLength={10}
                            ...
                            <View style={{ flex: 1, marginLeft: 20, marginRight: 20 }}>
                                ...
                                }}>{this.currencyFormat(totalBill)}</Text>
                    ...
                    <TouchableOpacity onPress={this.onPress} style={styles.button} >
                        {
                            this.state.processing ?
                                <Text style={styles.textGrey}>Đợi phản hồi từ MoMo App</Text>
                                :
